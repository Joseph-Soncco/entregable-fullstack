...
module.exports = (upload) => {
  ...
  // 1) LISTADO + BUSCADOR + FILTROS + ORDEN
  ...
  // 3) CREAR MANGA con Multer y recorte de precio
  router.post('/create', upload.single('imagenFile'), async (req, res) => {
    try {
      ...
      const imagen = req.file
        ...
        : null
      const { titulo, autor, idcategoria, ideditorial, idgenero } = req.body

      await db.query(
        `INSERT INTO mangas
           (idcategoria, ideditorial, idgenero, titulo, autor, precio, imagen)
         VALUES (?,?,?,?,?,?,?)`,
        [idcategoria, ideditorial, idgenero, titulo, autor, precioNum, imagen]
      )
      res.redirect('/mangas')
    } catch (error) {
      console.error(error)
  ...
  router.get('/edit/:id', async (req, res) => {
    try {
      const [[manga]] = await db.query(
        'SELECT * FROM mangas WHERE idmanga = ?',
        [req.params.id]
      )
  ...
  // 5) ACTUALIZAR MANGA con Multer y corrección SQL
  router.post('/edit/:id', upload.single('imagenFile'), async (req, res) => {
    try {
      let precioNum = parseFloat(req.body.precio) || 0
      if (precioNum > 300) precioNum = 300
      ...
      await db.query(
        `UPDATE mangas
           SET idcategoria  = ?,
               ...
               imagen       = ?
         WHERE idmanga = ?`,
        [idcategoria, ideditorial, idgenero, titulo, autor, precioNum, imagen, req.params.id]
      )
      res.redirect('/mangas')
    } catch (error) {
      console.error(error)
